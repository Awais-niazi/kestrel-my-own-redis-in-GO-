...
// writeLog produces a log file holding the given commands and returns its
// path and final stream offset.
func writeLog(t *testing.T, cmds ...[][]byte) (string, uint64) {
	t.Helper()
	path := filepath.Join(t.TempDir(), "kestrel.log")
	l, err := Create(Options{Path: path})
	if err != nil {
		t.Fatal(err)
	}
	for _, c := range cmds {
...
func TestRecoverCleanLog(t *testing.T) {
	...
	var c collector
	res, err := Recover(RecoverOptions{Path: path}, &c)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecoverMissingLog(t *testing.T) {
	var c collector
	_, err := Recover(RecoverOptions{Path: filepath.Join(t.TempDir(), "absent")}, &c)
...
func TestRecoverTruncatesTornTail(t *testing.T) {
	path, _ := writeLog(t, cmd("SET", "a", "1"), cmd("SET", "bbbbbbbbbb", "2"))
	...
	var c collector
	res, err := Recover(RecoverOptions{Path: path, Policy: PolicyTruncate}, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	var again collector
	res2, err := Recover(RecoverOptions{Path: path}, &again)
...
func TestRecoverRefusesTornTail(t *testing.T) {
	...
	var c collector
	_, err = Recover(RecoverOptions{Path: path, Policy: PolicyRefuse}, &c)
	if err == nil {
...
func TestRecoverRefusesCorruptRecord(t *testing.T) {
	path, _ := writeLog(t, cmd("SET", "a", "1"), cmd("SET", "b", "2"))
	raw, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecoverStopsOnDivergence(t *testing.T) {
	path, _ := writeLog(t, cmd("SET", "a", "1"), cmd("LPUSH", "a", "x"), cmd("SET", "c", "3"))
	before, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecoverRejectsForeignFile(t *testing.T) {
	...
	var c collector
	if _, err := Recover(RecoverOptions{Path: path}, &c); !errors.Is(err, ErrBadHeader) {
...
func TestRecoveredOffsetContinuesTheStream(t *testing.T) {
	path, end := writeLog(t, cmd("SET", "a", "1"), cmd("SET", "b", "2"))
	var c collector
	res, err := Recover(RecoverOptions{Path: path}, &c)
	if err != nil {
		t.Fatal(err)
